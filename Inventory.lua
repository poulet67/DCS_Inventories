...
function Delivery_Handoff(arrivingAirbase, UnitAttachedto)
...
function Inventory_Add_Weapon(WeaponName, InvAirbase, quantitytoAdd)
...
function Inventory_Add_Fuel(InvAirbase, quantitytoAdd)
...
function Inventory_Add_GroundUnit(UnitName, InvAirbase, quantitytoAdd)

-- not yet implemented

end

function Inventory_Check(playerUnit, currentAirbase)
...
function Inventory_Find_Weapon_Qty(WeaponName, currentAirbase)
...
function getCurrentAirbase(x_current,y_current,z_current) --Gets the closest airbase to a set of x y z coordinates
	...
	for i = 1, #base do
		...
		mag = math.sqrt(diffx^2 + diffy^2 + diffz^2) 
...
function print_to_JSON()
    ...
    trigger.action.outText("SHEDULED", 30)	
	
 	file = io.open(lfs.writedir().."\\Scripts\\Inventories System\\Inventory\\inventories_state.JSON", "w+")
